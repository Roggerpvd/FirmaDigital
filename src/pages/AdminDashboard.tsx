...
"Hash del documento (SHA-256)": d.documentHash ??"",
 ...
 return (
 <div className="min-h-screen transition-colors duration-200 bg-transparent text-on-surface">
 {/* Toast */}
 {toast && (
 <div className="fixed top-5 right-5 z-[100] max-w-sm rounded-lg shadow-lg border border-outline-variant p-md flex items-start gap-md bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md">
 ...
 <p className="font-body-md text-[13px] text-on-surface-variant mt-xs leading-normal">{toast.message}</p>
 </div>
 ...
 {/* Sidebar */}
 <aside className={`w-[280px] h-screen fixed left-0 top-0 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border-r border-outline-variant flex flex-col py-lg px-md z-50 transition-transform duration-300 md:translate-x-0 ${sidebarOpen ?"translate-x-0" :"-translate-x-full"}`}>
 <div className="mb-xl px-sm flex items-center justify-between">
 <div>
 <h1 className="font-headline-sm text-headline-sm font-bold text-primary">Mister Pan</h1>
 <p className="font-label-md text-label-md text-on-surface-variant tracking-wider uppercase opacity-70">Gestor de boletas de pago</p>
 </div>
 ...
 className={`flex items-center gap-md py-md px-md rounded-lg transition-colors cursor-pointer ${activeTab === item.key ?"text-primary font-bold border-r-2 border-primary bg-surface-container-low" :"text-on-surface-variant hover:bg-surface-container-low"}`}
 >
 ...
 {/* Header */}
 <header className="h-[72px] fixed top-0 right-0 w-full md:w-[calc(100%-280px)] bg-surface/10 backdrop-blur-md backdrop-blur-md border-b border-outline-variant flex items-center justify-between px-xl z-40 transition-colors duration-200">
 <div className="flex items-center gap-md">
 <button onClick={() => setSidebarOpen(true)} className="md:hidden material-symbols-outlined text-on-surface-variant hover:text-primary">menu</button>
 <div className="flex items-center bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-full px-md py-sm w-48 sm:w-96 transition-all duration-200 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary">
 <span className="material-symbols-outlined text-on-surface-variant mr-sm">search</span>
 <input
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 className="bg-transparent border-none focus:outline-none focus:ring-0 text-body-md font-body-md w-full placeholder-on-surface-variant"
 ...
 <div className="w-10 h-10 rounded-full bg-surface-container-highest/10 backdrop-blur-md backdrop-blur-md overflow-hidden border border-outline-variant flex items-center justify-center">
 ...
 <p className="text-[10px] uppercase font-bold text-on-surface-variant tracking-tighter">Cuenta admin</p>
 </div>
 </div>
 ...
 className="flex items-center gap-xs text-on-surface-variant hover:text-error transition-all font-body-md text-body-md"
 title="Cerrar sesión"
 >
 ...
 <div>
 <h2 className="font-headline-md text-headline-md text-primary">Boletas de pago</h2>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Gestiona y da seguimiento a las firmas de boletas de pago de tus empleados.</p>
 </div>
 <div className="flex gap-md">
 <div className="relative">
 <button onClick={() => { setMonthOpen(!monthOpen); setSortOpen(false); }} className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant px-lg py-md rounded-lg font-body-md text-body-md hover:bg-surface-container/10 backdrop-blur-md transition-colors flex items-center gap-sm active:scale-95 text-primary">
 <span className="material-symbols-outlined text-[20px]">calendar_today</span>
 {monthFilter}
 </button>
 {monthOpen && (
 <div className="absolute right-0 mt-sm w-48 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg shadow-xl z-50 overflow-hidden">
 {["Todos los meses", ...uniqueMonths].map(m => (
 <div key={m} onClick={() => { setMonthFilter(m); setMonthOpen(false); }} className="px-md py-sm text-[13px] cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md text-on-surface">{m}</div>
 ))}
 </div>
 )}
 </div>
 ...
 <section className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl overflow-hidden shadow-sm">
 <div className="px-xl pt-lg border-b border-outline-variant flex items-center justify-between">
 <div className="flex gap-xl overflow-x-auto">
 <button onClick={() => setFilterTab("all")} className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab ==="all" ?"border-primary text-primary font-semibold" :"border-transparent text-on-surface-variant hover:text-primary"}`}>
 Todos los Documentos ({documents.length})
 </button>
 <button onClick={() => setFilterTab("pending")} className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab ==="pending" ?"border-primary text-primary font-semibold" :"border-transparent text-on-surface-variant hover:text-primary"}`}>
 Pendientes ({countPending})
 </button>
 <button onClick={() => setFilterTab("signed")} className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab ==="signed" ?"border-primary text-primary font-semibold" :"border-transparent text-on-surface-variant hover:text-primary"}`}>
 Firmados ({countSigned})
 </button>
 </div>
 <div className="pb-md relative">
 <button onClick={() => { setSortOpen(!sortOpen); setMonthOpen(false); }} className="material-symbols-outlined text-on-surface-variant hover:text-primary p-xs rounded-md hover:bg-surface-container/10 backdrop-blur-md transition-colors">
 filter_list
 </button>
 {sortOpen && (
 <div className="absolute right-0 mt-sm w-52 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg shadow-xl z-50 overflow-hidden">
 {[
 { key:"recent", label:"Más reciente" },
 { key:"name-asc", label:"Nombre A-Z" },
 { key:"name-desc", label:"Nombre Z-A" }
 ].map(opt => (
 <div key={opt.key} onClick={() => { setSortOrder(opt.key as typeof sortOrder); setSortOpen(false); }} className={`px-md py-sm text-[13px] cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md ${sortOrder === opt.key ?"text-primary font-semibold" :"text-on-surface"}`}>
 {opt.label}
 </div>
 ))}
 </div>
 ...
 <thead>
 <tr className="bg-surface-container-low/10 backdrop-blur-md backdrop-blur-md text-left border-b border-outline-variant">
 <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Empleado</th>
 <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">ID de Boleta</th>
 <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Estado</th>
 <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Fecha de Firma</th>
 <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-right">Acciones</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-outline-variant">
 {paginatedDocuments.length > 0 ? (
 paginatedDocuments.map((doc) => (
 <tr key={doc.id} className="hover:bg-surface-container-low/10 backdrop-blur-md transition-colors">
 <td className="px-xl py-md">
 <div className="flex items-center gap-md">
 <div className="w-8 h-8 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center font-bold text-label-md overflow-hidden">
 ...
 <span className="text-[11px] text-outline">{doc.email}</span>
 </div>
 </div>
 </td>
 <td className="px-xl py-md">
 <span className="font-data-mono text-data-mono text-on-surface-variant">{doc.payslipId}</span>
 </td>
 <td className="px-xl py-md">
 {doc.status ==="Signed" ? (
 <div className="inline-flex items-center gap-sm px-md py-xs rounded-full bg-emerald-50 text-emerald-700 text-label-md font-semibold border border-emerald-100">
 ...
 <td className="px-xl py-md">
 <span className="font-body-md text-body-md text-on-surface-variant">{doc.date}</span>
 </td>
 <td className="px-xl py-md text-right">
 <div className="flex items-center justify-end gap-xs">
 <button
 onClick={() => handleDownloadPdf(doc.payslipId)}
 className="material-symbols-outlined text-primary hover:bg-surface-container/10 backdrop-blur-md rounded-lg p-sm transition-all text-[20px] active:scale-90"
 title="Descargar PDF"
 >
 picture_as_pdf
 </button>
 ...
 className="material-symbols-outlined text-emerald-600 hover:bg-surface-container/10 backdrop-blur-md rounded-lg p-sm transition-all text-[20px] active:scale-90"
 ...
 <div className="px-xl py-lg bg-surface/10 backdrop-blur-md backdrop-blur-md border-t border-outline-variant flex flex-col sm:flex-row gap-md items-center justify-between">
 ...
 <div className="flex items-center gap-sm">
 <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="w-10 h-10 flex items-center justify-center rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container/10 backdrop-blur-md transition-colors disabled:opacity-30">
 <span className="material-symbols-outlined">chevron_left</span>
 </button>
 <div className="flex gap-xs">
 {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
 <button key={p} onClick={() => setCurrentPage(p)} className={`w-10 h-10 flex items-center justify-center rounded-lg font-bold transition-colors ${currentPage === p ?"bg-primary text-on-primary" :"border border-outline-variant text-on-surface-variant hover:bg-surface-container/10 backdrop-blur-md"}`}>
 {p}
 </button>
 ))}
 </div>
 <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="w-10 h-10 flex items-center justify-center rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container/10 backdrop-blur-md transition-colors disabled:opacity-30">
 ...
 <div className="mb-xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-md">
 <div>
 <h2 className="font-headline-md text-headline-md text-primary">Empleados</h2>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Directorio de empleados y su historial de firmas.</p>
 </div>
 <div className="flex gap-md items-center">
 <div className="flex items-center bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-full px-md py-sm w-56">
 <span className="material-symbols-outlined text-on-surface-variant mr-sm text-[18px]">search</span>
 <input value={employeeSearch} onChange={(e) => setEmployeeSearch(e.target.value)} className="bg-transparent border-none focus:outline-none focus:ring-0 text-body-md font-body-md w-full" placeholder="Buscar empleado..." type="text" />
 </div>
 <button onClick={() => setShowEmployeeModal(true)} className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 transition-all flex items-center gap-sm shadow-sm">
 ...
 return (
 <div key={emp.email} className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-lg shadow-sm">
 ...
 <p className="font-body-md text-body-md font-semibold text-primary truncate">{emp.full_name}</p>
 <p className="text-[11px] text-outline truncate">{emp.email}</p>
 <p className="text-[11px] text-outline">{emp.employee_code}{emp.position ?` · ${emp.position}` :""}</p>
 </div>
 </div>
 <div className="flex items-center justify-between">
 <div className="flex gap-md text-[12px]">
 <span className="text-emerald-600 font-semibold">{stats?.firmados ?? 0} firmadas</span>
 <span className="text-amber-600 font-semibold">{stats?.pendientes ?? 0} pendientes</span>
 </div>
 <Link
 to={`/mis-boletas/${emp.employee_code}`}
 className="text-[11px] font-semibold text-primary hover:underline flex items-center gap-xs shrink-0"
 >
 Ver portal
 <span className="material-symbols-outlined text-[14px]">open_in_new</span>
 </Link>
 </div>
 <div className="mt-md pt-md border-t border-outline-variant flex justify-end gap-lg">
 <button
 onClick={() => handleResetEmployeePassword(emp.employee_code, emp.full_name)}
 disabled={resettingEmployeeCode === emp.employee_code}
 className="text-[11px] font-semibold text-primary hover:underline flex items-center gap-xs disabled:opacity-50"
 >
 ...
 className="text-[11px] font-semibold text-error hover:underline flex items-center gap-xs disabled:opacity-50"
 >
 ...
 {employees.length === 0 && (
 <p className="text-on-surface-variant col-span-full text-center py-xl">No hay empleados registrados todavía.</p>
 )}
 </div>
 </>
 )}
 ...
 <>
 <div className="mb-xl">
 <h2 className="font-headline-md text-headline-md text-primary">Reportes</h2>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Exporta el estado de firmas de todas las boletas de pago.</p>
 </div>
 <div className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-xl shadow-sm mb-xl">
 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-md">
 <div>
 <h3 className="font-headline-sm text-headline-sm text-primary">Reporte de Boletas (Excel)</h3>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Incluye {documents.length} registros, uno por empleado: nombre, código, correo, periodo, estado, IP, dispositivo y hash de firma.</p>
 </div>
 <button onClick={handleExportExcel} className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 active:scale-95 transition-all flex items-center gap-sm shadow-sm">
 ...
 <div className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-lg shadow-sm text-center">
 <p className="text-2xl font-bold text-primary">{documents.length}</p>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Boletas totales</p>
 </div>
 <div className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-lg shadow-sm text-center">
 <p className="text-2xl font-bold text-emerald-600">{countSigned}</p>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Firmadas</p>
 </div>
 <div className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-lg shadow-sm text-center">
 <p className="text-2xl font-bold text-amber-600">{countPending}</p>
 <p className="font-body-md text-body-md text-on-surface-variant mt-xs">Pendientes</p>
 </div>
 </div>
 </>
 )}
 ...
 {showBatchModal && (
 <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99] flex items-center justify-center p-md">
 <div className="bg-surface-container-lowest/70 backdrop-blur-xl border border-white/60 w-full max-w-md rounded-xl p-xl shadow-2xl ring-1 ring-black/5">
 ...
 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Empleado *</label>
 <select
 required
 value={newPayslipEmail}
 onChange={(e) => setNewPayslipEmail(e.target.value)}
 className="w-full bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg px-md py-sm font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
 >
 ...
 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Archivo PDF de la Boleta *</label>
 <label className="custom-dashed h-32 rounded-lg flex flex-col items-center justify-center gap-sm cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md transition-colors relative">
 <input
 type="file"
 ...
 <button type="button" onClick={() => setShowBatchModal(false)} className="bg-surface/10 backdrop-blur-md backdrop-blur-md border border-outline-variant px-lg py-md rounded-lg font-body-md text-body-md text-primary hover:bg-surface-container/10 backdrop-blur-md transition-colors">
 Cancelar
 </button>
 <button type="submit" disabled={creatingBatch} className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 transition-colors shadow-sm disabled:opacity-50">
 ...
 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Código de Empleado</label>
 ...
 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Nombre completo *</label>
 <input type="text" required value={newEmployeeName} onChange={(e) => setNewEmployeeName(e.target.value)} placeholder="ej. María Fernanda Quispe" className="w-full bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg px-md py-sm font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
 </div>

 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Correo Electrónico *</label>
 <input type="email" required value={newEmployeeEmail} onChange={(e) => setNewEmployeeEmail(e.target.value)} placeholder="ej. empleado@empresa.pe" className="w-full bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg px-md py-sm font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
 </div>

 <div>
 <label className="block text-[12px] font-bold uppercase tracking-wider text-on-surface-variant mb-xs">Puesto</label>
 <input type="text" value={newEmployeePosition} onChange={(e) => setNewEmployeePosition(e.target.value)} placeholder="ej. Asistente Administrativo" className="w-full bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg px-md py-sm font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
 </div>

 <div className="pt-md flex items-center justify-end gap-md">
 <button type="button" onClick={() => setShowEmployeeModal(false)} className="bg-surface/10 backdrop-blur-md backdrop-blur-md border border-outline-variant px-lg py-md rounded-lg font-body-md text-body-md text-primary hover:bg-surface-container/10 backdrop-blur-md transition-colors">
 Cancelar
 </button>
 <button type="submit" disabled={creatingEmployee} className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 transition-colors shadow-sm disabled:opacity-50">
