...
const statusLabel = (status: "Signed" | "Pending") =>
...
function AdminDashboard({ adminFullName }: AdminDashboardProps) {
  ...
  const employeesList = Object.values(
    documents.reduce(
      ...
      {} as Record<
        string,
        { name: string; email: string; total: number; firmados: number; pendientes: number }
      >,
    ),
  ).filter(
    (emp) =>
    ...
    return (
      <div className="min-h-screen bg-transparent text-on-surface flex items-center justify-center p-md">
        <div className="max-w-sm text-center bg-surface-container-lowest/10 backdrop-blur-md border border-outline-variant rounded-xl p-xl shadow-sm">
          ...
          <button
            onClick={loadData}
            className="text-primary text-[13px] font-semibold hover:underline">Reintentar</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen transition-colors duration-200 bg-transparent text-on-surface">
      ...
      {toast && (
        <div className="fixed top-5 right-5 z-[100] max-w-sm rounded-lg shadow-lg border border-outline-variant p-md flex items-start gap-md bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md">
          <span
            className={`material-symbols-outlined ${toast.type === "success" ? "text-emerald-500" : toast.type === "error" ? "text-error" : "text-sky-500"}`}
          >
          ...
          <div className="flex-1">
            ...
            <p className="font-body-md text-[13px] text-on-surface-variant mt-xs leading-normal">
              {toast.message}
            </p>
          </div>
          <button
            onClick={() => setToast(null)}
            className="material-symbols-outlined text-[18px] text-outline hover:text-primary transition-colors"
          >
            close
          </button>
        </div>
      ...
      {generatedPassword && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99] flex items-center justify-center p-md">
          <div className="bg-surface-container-lowest/70 backdrop-blur-xl border border-white/60 w-full max-w-md rounded-xl p-xl shadow-2xl ring-1 ring-black/5 text-center">
            <div
              className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-lg ${generatedPassword.emailSent ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
            >
              <span className="material-symbols-outlined text-[28px]">
                {generatedPassword.emailSent ? "mark_email_read" : "warning"}
              </span>
            </div>
            <h3 className="font-headline-sm text-headline-sm font-bold text-primary mb-xs">
              {generatedPassword.title}
            </h3>
            ...
            {!generatedPassword.emailSent && (
              <div className="bg-surface-container/60 backdrop-blur-xl border border-white/40 rounded-lg p-md mb-lg text-left space-y-xs shadow-inner">
                ...
                <p className="font-data-mono text-data-mono text-primary text-[16px] font-bold">
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 bg-black/40 backdrop-blur-sm backdrop-blur-sm z-40 md:hidden"
      ...
      {/* Sidebar */}
      <aside
        className={`w-[280px] h-screen fixed left-0 top-0 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border-r border-outline-variant flex flex-col py-lg px-md z-50 transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="mb-xl px-sm flex items-center justify-between">
          <div>
            ...
            <p className="font-label-md text-label-md text-on-surface-variant tracking-wider uppercase opacity-70">
              Gestor de boletas de pago
            </p>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
            className="md:hidden material-symbols-outlined text-outline hover:text-primary"
          >
            close
          </button>
        </div>
      ...
      <header className="h-[72px] fixed top-0 right-0 w-full md:w-[calc(100%-280px)] bg-surface/10 backdrop-blur-md backdrop-blur-md border-b border-outline-variant flex items-center justify-between px-xl z-40 transition-colors duration-200">
        <div className="flex items-center gap-md">
          <button
            ...
            className="md:hidden material-symbols-outlined text-on-surface-variant hover:text-primary"
          >
            menu
          </button>
              ...
              <div className="mb-xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-md">
                <div>
                  ...
                  <p className="font-body-md text-body-md text-on-surface-variant mt-xs">
                    Gestiona y da seguimiento a las firmas de boletas de pago de tus empleados.
                  </p>
                </div>
                <div className="flex gap-md">
                  <div className="relative">
                    <button
                      ...
                      className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant px-lg py-md rounded-lg font-body-md text-body-md hover:bg-surface-container/10 backdrop-blur-md transition-colors flex items-center gap-sm active:scale-95 text-primary"
                    >
                      ...
                      <div className="absolute right-0 mt-sm w-48 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg shadow-xl z-50 overflow-hidden">
                        {["Todos los meses", ...uniqueMonths].map((m) => (
                          <div
                            ...
                            className="px-md py-sm text-[13px] cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md text-on-surface"
                          >
                            {m}
                          </div>
                        ))}
                ...
                <div className="px-xl pt-lg border-b border-outline-variant flex items-center justify-between">
                  <div className="flex gap-xl overflow-x-auto">
                    <button
                      ...
                      className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab === "all" ? "border-primary text-primary font-semibold" : "border-transparent text-on-surface-variant hover:text-primary"}`}
                    >
                      Todos los Documentos ({documents.length})
                    </button>
                    <button
                      onClick={() => setFilterTab("pending")}
                      className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab === "pending" ? "border-primary text-primary font-semibold" : "border-transparent text-on-surface-variant hover:text-primary"}`}
                    >
                      Pendientes ({countPending})
                    </button>
                    <button
                      onClick={() => setFilterTab("signed")}
                      className={`pb-md border-b-2 font-body-md text-body-md whitespace-nowrap transition-all ${filterTab === "signed" ? "border-primary text-primary font-semibold" : "border-transparent text-on-surface-variant hover:text-primary"}`}
                    >
                  ...
                  <div className="pb-md relative">
                    <button
                      ...
                      className="material-symbols-outlined text-on-surface-variant hover:text-primary p-xs rounded-md hover:bg-surface-container/10 backdrop-blur-md transition-colors"
                    >
                      filter_list
                    </button>
                    {sortOpen && (
                      <div className="absolute right-0 mt-sm w-52 bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg shadow-xl z-50 overflow-hidden">
                        {[
                          ...
                          <div
                            key={opt.key}
                            onClick={() => {
                              setSortOrder(opt.key as typeof sortOrder);
                              setSortOpen(false);
                            }}
                            className={`px-md py-sm text-[13px] cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md ${sortOrder === opt.key ? "text-primary font-semibold" : "text-on-surface"}`}
                          >
                            {opt.label}
                          </div>
                        ))}
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-surface-container-low/10 backdrop-blur-md backdrop-blur-md text-left border-b border-outline-variant">
                        <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">
                          Empleado
                        </th>
                        <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">
                          ID de Boleta
                        ...
                        <th className="px-xl py-lg font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-right">
                          Acciones
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-outline-variant">
                      {paginatedDocuments.length > 0 ? (
                        paginatedDocuments.map((doc) => (
                          <tr
                            key={doc.id}
                            className="hover:bg-surface-container-low/10 backdrop-blur-md transition-colors"
                          >
                  ...
                  <div className="px-xl py-lg bg-surface/10 backdrop-blur-md backdrop-blur-md border-t border-outline-variant flex flex-col sm:flex-row gap-md items-center justify-between">
                    ...
                    <div className="flex items-center gap-sm">
                      <button
                        ...
                        className="w-10 h-10 flex items-center justify-center rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container/10 backdrop-blur-md transition-colors disabled:opacity-30"
                      >
                      ...
                      <div className="flex gap-xs">
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                          <button
                            ...
                            className={`w-10 h-10 flex items-center justify-center rounded-lg font-bold transition-colors ${currentPage === p ? "bg-primary text-on-primary" : "border border-outline-variant text-on-surface-variant hover:bg-surface-container/10 backdrop-blur-md"}`}
                          >
                            {p}
                          </button>
                        ))}
            ...
            <>
              <div className="mb-xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-md">
                <div>
                  ...
                  <p className="font-body-md text-body-md text-on-surface-variant mt-xs">
                    Directorio de empleados y su historial de firmas.
                  </p>
                </div>
                <div className="flex gap-md items-center">
                  <div className="flex items-center bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-full px-md py-sm w-56">
                    ...
                    <input
                      ...
                      className="bg-transparent border-none focus:outline-none focus:ring-0 text-body-md font-body-md w-full"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-md mb-xl">
                {employees
                  ...
                  .map((emp) => {
                    ...
                    return (
                      <div
                        key={emp.email}
                        className="bg-surface-container-lowest/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-xl p-lg shadow-sm"
                      >
                        ...
                        <div className="mt-md pt-md border-t border-outline-variant flex justify-end gap-lg">
                          <button
                            onClick={() =>
                              handleResetEmployeePassword(emp.employee_code, emp.full_name)
                            }
                          ...
                          >
                            ...
                            {resettingEmployeeCode === emp.employee_code
                              ? "Restableciendo..."
                          ...
                          >
                            ...
                            {deletingEmployeeCode === emp.employee_code
                              ? "Eliminando..."
                              : "Eliminar empleado"}
                          </button>
                        </div>
                ...
                {employees.length === 0 && (
                  <p className="text-on-surface-variant col-span-full text-center py-xl">
          ...
          {activeTab === "reports" && (
            <>
              <div className="mb-xl">
                ...
                <p className="font-body-md text-body-md text-on-surface-variant mt-xs">
                  Exporta el estado de firmas de todas las boletas de pago.
                </p>
              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-md">
                  <div>
                    <h3 className="font-headline-sm text-headline-sm text-primary">
                      Reporte de Boletas (Excel)
                    </h3>
                    <p className="font-body-md text-body-md text-on-surface-variant mt-xs">
                      Incluye {documents.length} registros, uno por empleado: nombre, código,
                      correo, periodo, estado, IP, dispositivo y hash de firma.
                    </p>
                  </div>
                  <button
                    onClick={handleExportExcel}
                    className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 active:scale-95 transition-all flex items-center gap-sm shadow-sm"
                  >
          ...
          <div className="bg-surface-container-lowest/70 backdrop-blur-xl border border-white/60 w-full max-w-md rounded-xl p-xl shadow-2xl ring-1 ring-black/5">
            ...
            <form onSubmit={handleCreatePayslip} className="space-y-md">
              ...
              <p className="text-[11px] text-on-surface-variant -mt-sm">
                El período y la fecha se precargan con la fecha de hoy (hora Perú), pero puedes
                editarlos manualmente. El ID de la boleta se genera solo, en orden, para el empleado
                seleccionado.
              </p>
              ...
              <div>
                ...
                <label className="custom-dashed h-32 rounded-lg flex flex-col items-center justify-center gap-sm cursor-pointer hover:bg-surface-container-low/10 backdrop-blur-md transition-colors relative">
                  ...
                  />
                  <span className="material-symbols-outlined text-[32px] text-outline opacity-40">
                    picture_as_pdf
                  </span>
              ...
              <div className="pt-md flex items-center justify-end gap-md">
                <button
                  type="button"
                  onClick={() => setShowBatchModal(false)}
                  className="bg-surface/10 backdrop-blur-md backdrop-blur-md border border-outline-variant px-lg py-md rounded-lg font-body-md text-body-md text-primary hover:bg-surface-container/10 backdrop-blur-md transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="bg-primary text-on-primary px-lg py-md rounded-lg font-body-md text-body-md hover:opacity-90 transition-colors shadow-sm disabled:opacity-50"
                >
            ...
            <form onSubmit={handleCreateEmployee} className="space-y-md">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="ej. empleado@empresa.pe"
                  className="w-full bg-surface-container/10 backdrop-blur-md backdrop-blur-md border border-outline-variant rounded-lg px-md py-sm font-body-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                />
              </div>
